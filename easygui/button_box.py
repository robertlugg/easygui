...
class ButtonBox(AbstractBox):
    ...
    def __init__(self, msg, title, choices, images, default_choice, cancel_choice, callback):
        ...
        self.msg = msg
        self.cancel_value = cancel_choice
    ...
    def configure_message_widget(self, monospace):
        """ Overriding the base class - we cannot mix pack() and grid() so this message widget uses pack() """
    ...
    def _create_images_frame(self, filenames):
        ...
        filename_array = convert_to_a_list_of_lists(filenames)
        for row, list_of_filenames in enumerate(filename_array):
            for column, filename in enumerate(list_of_filenames):
                ...
                # keep a reference to each image on self to prevent deletion
                self._images.append({'tk_image': tk_image, 'widget': widget})
        return images_frame
